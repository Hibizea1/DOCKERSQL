...
function showItemInfo(item) {
    if (!item) {
        ...
        document.getElementById("info-desc").textContent = "Description : Aucun item equipe.";
        return;
    }
    ...
    document.getElementById("info-desc").textContent = "Description : " + (item.description || "Aucune description.");
}
...
function fillStats(character) {
    ...
    document.getElementById("stat-strength").textContent = readStat("strength", "strenght");
    document.getElementById("stat-intelligence").textContent = readStat("intelligence");
}

function renderInventory(inventories, isVisible) {
    const table = document.getElementById("inventories");
    const body = table.querySelector("tbody");
    const hiddenMessage = document.getElementById("inventoryHiddenMessage");
...
async function getPlayerData() {
    try {
        ...
        if (data.status !== "success") {
            alert("Impossible de recuperer les infos du joueur !");
            return;
        }
        ...
        const canShowInventory = Number(data.params?.inventorypreview ?? 0) === 1;
        renderInventory(data.inventories, canShowInventory);
...
searchInput.addEventListener("input", async () => {
    ...
    try {
        ...
        if (data.status !== "success" || data.users.length === 0) {
            searchResults.innerHTML = "<div class='resultItem'>Aucun resultat</div>";
        } else {
...
function setupTopMenuDropdown() {
    if (!menuToggleButton || !viewerMenuDropdown) {
        return;
    }

    const setOpen = (isOpen) => {
        viewerMenuDropdown.classList.toggle("show", isOpen);
        menuToggleButton.setAttribute("aria-expanded", String(isOpen));
    };

    menuToggleButton.addEventListener("click", (event) => {
        event.stopPropagation();
        setOpen(!viewerMenuDropdown.classList.contains("show"));
    });

    document.addEventListener("click", (event) => {
        if (!viewerMenuDropdown.contains(event.target) && event.target !== menuToggleButton) {
            setOpen(false);
        }
    });
